...
| GPU demo | `zig build run_gpu -Dgpu=metal` | `nnctl run gpu --gpu metal` | Metal-backed matrix operations on macOS |
| GPU benchmark | `zig build run_gpu_benchmark -Dgpu=metal -Doptimize=ReleaseFast` | `nnctl run gpu-benchmark` | CPU vs Metal matrix multiplication timing |
