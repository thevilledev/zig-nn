...
const implementation = compiled_gpu.Implementation(struct {
    pub const Driver = CUDA;
    pub const Error = CUDAError;
    pub const enabled = enable_cuda;
    pub const not_enabled = error.CUDANotEnabled;
    pub const not_supported = error.CUDANotSupported;
    pub const device_not_found = error.CUDADeviceNotFound;
    pub const initialization_failed = error.CUDAInitializationFailed;
    pub const backend_type = backend.BackendType.CUDA;
});

pub const CUDABackend = implementation.GpuBackend;
