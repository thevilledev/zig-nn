...
const CUDA = if (enable_cuda) struct {
    ...
    pub fn launchPermuteBatchHeads(backend_ref: *Backend, input: *Buffer, result: *Buffer, batch: u32, tokens: u32, heads: u32, width: u32, split: bool) bool {
        return c.cuda_launch_permute_batch_heads(toC(backend_ref), toC(input), toC(result), batch, tokens, heads, width, @intFromBool(split)) != 0;
    }
    ...
    pub fn launchOptimizerUpdate(backend_ref: *Backend, parameter: *Buffer, gradient: *Buffer, first_moment: *Buffer, second_moment: *Buffer, total_squares: *Buffer, config: backend.OptimizerUpdateConfig, size: u32) bool {
        return c.cuda_launch_optimizer_update(toC(backend_ref), toC(parameter), toC(gradient), toC(first_moment), toC(second_moment), toC(total_squares), @intFromEnum(config.kind), size, config.learning_rate, config.beta1, config.beta2, config.epsilon, config.weight_decay, config.bias_correction1, config.bias_correction2, config.max_gradient_norm) != 0;
    }
...
pub const CUDABackend = struct {
    ...
    fn dispatchPermuteBatchHeads(self: *CUDABackend, input: *const Matrix, result: *Matrix, batch: usize, tokens: usize, heads: usize, width: usize, split: bool) bool {
        const backend_ref = self.backend orelse return false;
        ...
        if (!CUDA.launchPermuteBatchHeads(backend_ref, source_buffer, destination_buffer, toU32(batch), toU32(tokens), toU32(heads), toU32(width), split)) return false;
        if (!self.completeSubmittedKernel()) return false;
    ...
    fn dispatchOptimizerUpdate(self: *CUDABackend, parameter: *Matrix, gradient: *const Matrix, first_moment: *Matrix, second_moment: *Matrix, total_squares: *const Matrix, config: backend.OptimizerUpdateConfig) bool {
        const backend_ref = self.backend orelse return false;
        const parameter_data = getCUDAMatrix(parameter);
        const gradient_data = getCUDAMatrix(gradient);
        const first_moment_data = getCUDAMatrix(first_moment);
        const second_moment_data = getCUDAMatrix(second_moment);
        const total_squares_data = getCUDAMatrix(total_squares);
        ...
        if (!CUDA.launchOptimizerUpdate(backend_ref, parameter_buffer, gradient_buffer, first_moment_buffer, second_moment_buffer, total_squares_buffer, config, toU32(parameter.rows * parameter.cols))) return false;
        if (!self.completeSubmittedKernel()) return false;
