...
const DevicePreference = tensor_mod.DevicePreference;
const ExecutionContext = tensor_mod.ExecutionContext;
const Tensor = tensor_mod.Tensor;
const Transformer = transformer_mod;
const Decoding = decoding_mod;
const math = std.math;
...
pub const TinyGPT = struct {
    ...
    pub fn generateTokensDevice(
        ...
        var device = try Device.init(allocator, preference);
        defer device.deinit();
        var execution = ExecutionContext.init(&device);
    ...
    pub fn toDeviceDecoder(self: *const TinyGPT, context: *ExecutionContext) !Transformer.Decoder {
        var random_source = std.Random.DefaultPrng.init(0);
        var decoder = try Transformer.Decoder.init(context, .{
    ...
    pub fn syncFromDeviceDecoder(self: *TinyGPT, decoder: *const Transformer.Decoder) !void {
...
pub fn forwardDeviceTokens(
    allocator: std.mem.Allocator,
    decoder: *const Transformer.Decoder,
...
fn matrixToTensor(allocator: std.mem.Allocator, source: Matrix, target: *Tensor) !void {
...
fn tensorToMatrix(allocator: std.mem.Allocator, source: Tensor, target: *Matrix) !void {
...
fn linearToDevice(allocator: std.mem.Allocator, source: Linear, target: *Transformer.Linear) !void {
    try matrixToTensor(allocator, source.weights, &target.weights);
    try matrixToTensor(allocator, source.bias, &target.bias);
}

fn linearFromDevice(allocator: std.mem.Allocator, source: Transformer.Linear, target: *Linear) !void {
    try tensorToMatrix(allocator, source.weights, &target.weights);
    try tensorToMatrix(allocator, source.bias, &target.bias);
}

fn combinedQkvToDevice(allocator: std.mem.Allocator, source: Linear, target: *Transformer.CausalSelfAttention) !void {
    ...
    const projections = [_]*Transformer.Linear{ &target.query, &target.key, &target.value };
...
fn combinedQkvFromDevice(allocator: std.mem.Allocator, source: Transformer.CausalSelfAttention, target: *Linear) !void {
    ...
    const bias_values = try allocator.alloc(f32, channels);
    defer allocator.free(bias_values);
    const projections = [_]Transformer.Linear{ source.query, source.key, source.value };
    for (projections, 0..) |projection, part| {
